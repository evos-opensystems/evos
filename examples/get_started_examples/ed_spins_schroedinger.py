"""Solving the Schrödinger equation with exact diagonalization (ED) for a spin chain with algebraically decaying interactions and disorder."""
import evos
...
sz_1 = np.loadtxt('sz_1')

fig, axs = plt.subplots()

time_v = np.linspace(0, t_max, n_timesteps + 1  )
plt.plot(time_v,sz_0, label= 'sz_0')
plt.plot(time_v,sz_1, label= 'sz_1')

plt.legend()
fig.savefig('ed_spins_schroedinger.png')