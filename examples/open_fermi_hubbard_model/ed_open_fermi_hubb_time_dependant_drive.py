...
#import evos.src.methods.ed_time_dep_hamiltonian_lindblad_solver as ed_mesoscopic_leads
from scipy.integrate import solve_ivp
...
for t1 in range(len(t)):
    ...
    dyn = ed_mesoscopic_leads.MesoscopicLeadsLindblad(dim_H, H1[t1], L_list_left)
...
for t1 in t:
    if t1 == 0:
        t_before =0
        exp = n_up_1.dot(rho_updown).trace()
        rho_vec = rho_vec
    else:
        t_before_index = np.where(t==t1)[0] -1
        t_before = float(t[t_before_index])
    #print(int(np.where(t == t_before)[0]))
    
        dyn_drho_dt = Dyn_rho_dt[int(np.where(t == t_before)[0])]
        #print(dyn_drho_dt.drho_dt)
        #print(t1, t1+dt)
        
        sol = solve_ivp(dyn_drho_dt.drho_dt, (t_before,t_before+dt), rho_vec, t_eval = [t_before+dt])
    #print(np.shape(sol.y))
    

        #plot some expectation value at each time step
        #time dependant rho:
        rho_sol = np.zeros((dim_H,dim_H),dtype='complex')
        count=0
