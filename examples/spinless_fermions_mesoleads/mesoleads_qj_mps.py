...
#MAKE LATTICE
ferm_lat = ptn.mp.lat.nil.genFermiLattice(n_tot)
...
def H_leads_left( eps_vector_l, k_vector_l, mu_L ):
    ...
    for site in range( n_lead_left ): 
        # print( 'on site ', site)
        h_onsite_l.append( ( eps_vector_l[site] - mu_L ) *  ferm_lat.get('n', site ) )
    h_onsite_l = ptn.mp.addLog(h_onsite_l)
    h_onsite_l.truncate()
    ...
    for site in range(n_lead_left):
        ...
        h_sys_lead_l.append( k_vector_l[site] *  ( ferm_lat.get('c', n_lead_left ) * ferm_lat.get('ch',site ) + ferm_lat.get('c', site ) * ferm_lat.get('ch',n_lead_left ) ) )
    h_sys_lead_l = ptn.mp.addLog(h_sys_lead_l)
    h_sys_lead_l.truncate()    
    
    h_left_lead = h_onsite_l + h_sys_lead_l    
    h_left_lead.truncate()
    return h_left_lead
...
def H_leads_right( eps_vector_r, k_vector_r, mu_R ):
    ...
    for site in range( n_lead_left + n_system , n_tot ): 
        # print( 'on site ', site)
        h_onsite_r.append( ( eps_vector_r[ site - n_lead_left - n_system ] - mu_R ) * ferm_lat.get('n', site ) )
    h_onsite_r = ptn.mp.addLog(h_onsite_r)
    h_onsite_r.truncate()
    ...
    for site in range( n_lead_left + n_system , n_tot ):
        ...
        h_sys_lead_r.append( k_vector_r[ site - n_lead_left - n_system ] *  ( ferm_lat.get('c', n_system + n_lead_left -1 ) * ferm_lat.get('ch',site ) + ferm_lat.get('c', site ) * ferm_lat.get('ch',n_system + n_lead_left -1 ) ) )
    h_sys_lead_r = ptn.mp.addLog(h_sys_lead_r)
    h_sys_lead_r.truncate()
        
    h_right_lead = h_onsite_r + h_sys_lead_r 
    h_right_lead.truncate()
    return h_right_lead
...
def lindblad_op_list_left_lead( eps_delta_vector_l, eps_vector_l, mu_L, T_L ):
    l_list_left = []
    for site in range(n_lead_left):
        l_list_left.append( np.sqrt( eps_delta_vector_l[site] * np.exp( 1./T_L * ( eps_vector_l[site] - mu_L ) ) * fermi_dist( 1./T_L, eps_vector_l[site], mu_L ) ) * ferm_lat.get( 'c',site ) )
        l_list_left.append( np.sqrt( eps_delta_vector_l[site] * fermi_dist( 1./T_L, eps_vector_l[site], mu_L) ) * ferm_lat.get('ch',site) )
    return l_list_left

def lindblad_op_list_right_lead( eps_delta_vector_r, eps_vector_r, mu_R, T_R ):
    l_list_right = []
    for site in range(n_lead_right):
        l_list_right.append( np.sqrt( eps_delta_vector_r[site] * np.exp( 1./T_R * ( eps_vector_r[site] - mu_R ) ) * fermi_dist( 1./T_R, eps_vector_r[site], mu_R ) ) * ferm_lat.get( 'c', n_lead_left + n_system + site ) )
        l_list_right.append( np.sqrt( eps_delta_vector_r[site]* fermi_dist( 1./T_R, eps_vector_r[site], mu_R) ) * ferm_lat.get('ch', n_lead_left + n_system + site) )
    return l_list_right     
...
def H_sistem_t(A, om, dt, t_max, t ): 
    ...
    h_sys = eps * ferm_lat.get('c', n_lead_left) * ferm_lat.get('ch', n_lead_left)
    return h_sys 


def H_tot_t(t):
    #NOTE: this MUST a function of t only in order to be compatible with the time-dependent lindblad solver
    H_tot_t = H_leads_left + H_leads_right + H_sistem_t(A, om, dt, t_max, t ) 
    H_tot_t.truncate()
    return H_tot_t
...
vac =  ptn.mp.generateNearVacuumState(ferm_lat) #NOTE: this is not real vacuum

for i in range(n_tot): #create real vacuum
    vac *= ferm_lat.get( "c", i)
    vac.normalise()
    #print('exp value of n on ',i, ptn.mp.expectation( vac, ferm_lat.get('n',i) ))

#print('n_sys on vac: ', ptn.mp.expectation( vac, ferm_lat.get('n',n_lead_left) ) )

sys_up_state = vac.copy()
sys_up_state *= ferm_lat.get( 'ch', n_lead_left )

sys_up_state.normalise()
#print('n_sys on sys_up_state: ', ptn.mp.expectation( sys_up_state, ferm_lat.get('n',n_lead_left) ) )
init_state = (vac.copy() + sys_up_state.copy() )
init_state.normalise()
#print('n_sys on init_state: ', ptn.mp.expectation( init_state, ferm_lat.get('n',n_lead_left) ) )
...
j_p_left_op = []
for site in range(n_lead_left):
    j_p_left_op.append( k_vector_l[site] * ferm_lat.get('c',site)  * ferm_lat.get('ch',n_lead_left) )
    j_p_left_op.append( - k_vector_l[site] * ferm_lat.get('c',n_lead_left) * ferm_lat.get('ch',site) )
j_p_left_op = ptn.mp.addLog(j_p_left_op)    
j_p_left_op.truncate() 
j_p_left_op *= 1j
...
def compute_j_p_left_op(state, obs_array_shape,dtype):  #EXAMPLE 1D
    ...
    obs_array[0] = np.real( ptn.mp.expectation( state, j_p_left_op )) # / state.norm()
...
def compute_n_0(state, obs_array_shape,dtype):  #EXAMPLE 1D
    obs_array = np.zeros( obs_array_shape, dtype=dtype)
    #OBS DEPENDENT PART START
    obs_array[0] = np.real( ptn.mp.expectation( state, ferm_lat.get('n',0) ) ) #/ state.norm()  
    #OBS DEPENDENT PART END
    return obs_array
...
def compute_n_system(state, obs_array_shape,dtype):  #EXAMPLE 1D
    obs_array = np.zeros( obs_array_shape, dtype=dtype)
    #OBS DEPENDENT PART START
    obs_array[0] = np.real( ptn.mp.expectation( state, ferm_lat.get('n',n_lead_left) ) ) #/ state.norm() 
    #OBS DEPENDENT PART END
    return obs_array
...
#Quantum Jumps
qj = mps_quantum_jumps.MPSQuantumJumps( n_tot, ferm_lat ) 

try:
    os.mkdir( 'qj_mps_n_tot' + str(n_tot) )
except:
    pass    

os.chdir( 'qj_mps_n_tot' + str(n_tot) )
...
for trajectory in range(first_trajectory, n_trajectories + first_trajectory ): 
    ...
    for timestep in range(n_timesteps):
        state = qj.quantum_jump_single_trajectory_time_evolution(state, dt, dt, conf_tdvp, trajectory, obsdict,  H_tot_t( timestep_counter * dt ), l_list_tot, n_timesteps, compute_obs_for_init_state = False, timestep_for_obs_saving_shift = timestep_counter)
        timestep_counter += 1
...
obsdict.compute_trajectories_averages_and_errors( list(range(first_trajectory, n_trajectories + first_trajectory)), os.getcwd(), os.getcwd(), remove_single_trajectories_results=True ) #FIXME: remove time_gse_subspace_switch
