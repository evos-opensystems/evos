...
class DotWithOscillatorLattice():
    """"Creates the following lattice: fermion - fermion - boson - fermion, representing a left fermion lead (0), a quantum dot (1), a phononic mode (2), a right fermionic lead (3).
    ...
    def sso(self, operator_name: str, site: int) -> np.ndarray :
        ...
        if site == 0:
            ...
            single_site_operator = np.kron( self.Id_f, np.kron( self.Id_f, np.kron( operator.copy(), self.parity ) ) )    
        
        elif site == 3:
            single_site_operator = np.kron( self.Id_f, np.kron( self.Id_f, np.kron( self.Id_b, operator.copy() ) ) )          
