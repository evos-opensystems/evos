...
class EdQuantumJumps():
    ...
    def quantum_jump_single_trajectory_time_evolution(self, psi_t: np.ndarray, t_max: float, dt: float, trajectory: int, obsdict,  H: np.ndarray, lindbl_op_list: list, total_timesteps_for_r, compute_obs_for_init_state = True, timestep_for_obs_saving_shift = 0 ):
        ...
        for i in range( n_timesteps ):
            print('computing timestep ', i + timestep_shift + timestep_for_obs_saving_shift )
            ...
            if r1 > delta_p: #evolve with non-hermitian hamiltonian
                ...
                jump_time_list.append( i + timestep_shift + timestep_for_obs_saving_shift ) #debugging
                ...
                r2_atjump_list.append( r2_array[ i + timestep_shift + timestep_for_obs_saving_shift ] ) #debugging
